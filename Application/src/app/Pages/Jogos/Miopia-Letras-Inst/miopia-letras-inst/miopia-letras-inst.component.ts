...
@Component({
  selector: 'app-miopia-letras-inst',
  standalone: true,
  imports: [NgIf, NgFor, LogoMenuInstComponent],
  templateUrl: './miopia-letras-inst.component.html',
  styleUrl: './miopia-letras-inst.component.css'
})
export class MiopiaLetrasInstComponent {
  phases: PhaseConfig[] = [];
    ...
    if (this.currentPhaseIndex < this.phases.length - 1) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.router.navigate(['/instituicao/jogos/letras/result'], { state: { data: this.result } });
